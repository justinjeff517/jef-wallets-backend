...
def _get_all_accounts(table_name: str = ACCOUNTS_TABLE) -> dict:
    accounts = []
    start_key = None
...
def lambda_handler(event, context):
    try:
        out = _get_all_accounts()
        return _resp(200, out)
    except ClientError as e:
        msg = (e.response.get("Error") or {}).get("Message") or str(e)
        return _resp(500, {"exists": False, "message": f"DynamoDB error: {msg}", "accounts": []})
    except Exception as e:
        return _resp(500, {"exists": False, "message": f"Error: {str(e)}", "accounts": []})
