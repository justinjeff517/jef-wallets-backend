...
def _extract_payload(event: Dict[str, Any]) -> Dict[str, Any]:
    # Supports:
    # - direct invoke with dict payload
    # - API Gateway/Lambda Proxy with JSON string body
...
def get_all_transactions_by_account_number(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"exists": exists, "message": msg, "transactions": txs}
...
def lambda_handler(event, context):
    ...
    try:
        out = get_all_transactions_by_account_number(payload)
        return _resp(200, out)
    except Exception as e:
        return _resp(500, {"exists": False, "message": f"internal error: {str(e)}", "transactions": []})
